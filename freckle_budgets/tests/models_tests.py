...
class MonthTestCase(TestCase):
    ...
    def test_get_employee_public_holiday_days(self):
        fixtures.create_employee_project_months(self)
        result = self.month.get_employee_public_holiday_days(self.employee1)
        self.assertEqual(result.count(), 1, msg=(
            'Should return public holidays for given employee and month'))

    def test_get_employee_sick_leave_days(self):
        fixtures.create_employee_project_months(self)
        result = self.month.get_employee_sick_leave_days(self.employee1)
        self.assertEqual(result.count(), 1, msg=(
            'Should return sick leave days for given employee and month'))

    def test_get_employee_vacation_days(self):
        fixtures.create_employee_project_months(self)
        result = self.month.get_employee_vacation_days(self.employee1)
